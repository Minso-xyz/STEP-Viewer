...
public:
	...
	std::string GetCompleteEntity(int startLineIndex, std::vector<std::string> lines);
	...
	BSplineCurve ParseBSplineCurveWithKnots(const std::string line);
	...
	std::vector<std::string> SplitTopLevelParameters(const std::string& text);
	std::vector<Point3D> ParseControlPoints(std::string controlPointsBlock);
	std::vector<int> ParseMultiplicityNumbers(std::string multiplicityBlock);
	std::vector<double> ParseKnots(std::string knotBlock);
	std::vector<double> CalculateFinalKnots(std::vector<int> multiplicityNums, std::vector<double> knots);
	...
	std::vector<BSplineCurve> ExtractBSplineCurvesFromAllLines(std::vector<std::string> lines);
